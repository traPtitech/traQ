...
func newServer(hub2 *hub.Hub, db *gorm.DB, repo repository.Repository, fs storage.FileStorage, logger *zap.Logger, c2 *Config) (*Server, error) {
	...
	messageCounter := counter.NewMessageCounter(db, hub2)
	...
	threadCounter, err := counter.NewThreadCounter(db, hub2)
	if err != nil {
		return nil, err
	}
	...
	services := &service.Services{
		...
		WS:                   wsStreamer,
		BotWS:                streamer,
		QallRoomStateManager: roomStateManager,
		QallSoundBoard:       soundboard,
	}
