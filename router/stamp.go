...
// GetMyFavoriteStamps GET /users/me/favorite-stamps
func (h *Handlers) GetMyFavoriteStamps(c echo.Context) error {
	userID := getRequestUserID(c)

	ids, err := h.Repo.GetUserFavoriteStamps(userID)
	if err != nil {
		return internalServerError(err, h.requestContextLogger(c))
	}
...
// PostMyFavoriteStamp POST /users/me/favorite-stamps
func (h *Handlers) PostMyFavoriteStamp(c echo.Context) error {
	userID := getRequestUserID(c)
	...
	if err := h.Repo.AddFavoriteStamp(userID, req.StampID); err != nil {
		switch {
		case repository.ErrNilID == err:
			return badRequest("the stamp doesn't exist")
		case repository.IsArgError(err):
			return badRequest(err)
		default:
			return internalServerError(err, h.requestContextLogger(c))
...
// DeleteMyFavoriteStamp DELETE /users/me/favorite-stamps/:stampID
func (h *Handlers) DeleteMyFavoriteStamp(c echo.Context) error {
	userID := getRequestUserID(c)
	stampID := getRequestParamAsUUID(c, paramStampID)

	if err := h.Repo.RemoveFavoriteStamp(userID, stampID); err != nil {
		switch {
		case repository.ErrNilID == err:
			return badRequest("invalid stamp id")
		default:
			return internalServerError(err, h.requestContextLogger(c))
		}
	}
