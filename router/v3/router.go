	...
	api := e.Group("/v3", middlewares.UserAuthenticate(h.Repo))
	{
		apiUsers := api.Group("/users")
		{
			...
			apiUsersMe := apiUsers.Group("/me")
			{
				...
				apiUsersMe.GET("/subscription", h.GetMyChannelSubscriptions, requires(permission.GetChannelSubscription))
				apiUsersMe.PUT("/subscription/:channelID", h.SetChannelSubscribeLevel, requires(permission.EditChannelSubscription))
