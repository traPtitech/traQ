...
func TestRepositoryImpl_RemoveStampFromMessage(t *testing.T) {
	...
	t.Run("Nil id", func(t *testing.T) {
		...
		assert.EqualError(t, repo.RemoveStampFromMessage(message.ID, uuid.Nil, user.ID), repository.ErrNilID.Error())
		assert.EqualError(t, repo.RemoveStampFromMessage(uuid.Nil, stamp.ID, user.ID), repository.ErrNilID.Error())
	})
