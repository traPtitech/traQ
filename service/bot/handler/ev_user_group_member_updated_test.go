...
func TestUserGroupMemberUpdated(t *testing.T) {
	t.Parallel()

	b := &model.Bot{
		ID:              uuid.NewV3(uuid.Nil, "b"),
		BotUserID:       uuid.NewV3(uuid.Nil, "bu"),
		SubscribeEvents: model.BotEventTypesFromArray([]string{event.UserGroupMemberUpdated.String()}),
	...
	t.Run("success", func(t *testing.T) {
		...
		expectMulticast(handlerCtx, event.UserGroupMemberUpdated, payload.MakeUserGroupMemberUpdated(et, groupID, userID), []*model.Bot{b})
		assert.NoError(t, UserGroupMemberUpdated(handlerCtx, et, intevent.UserGroupMemberUpdated, hub.Fields{
			"group_id": groupID,
			"user_id":  userID,
		}))
	})

	t.Run("not subscribe USER_GROUP_MEMBER_UPDATED", func(t *testing.T) {
		t.Parallel()
		ctrl := gomock.NewController(t)
		handlerCtx := mock_handler.NewMockContext(ctrl)
		b2 := &model.Bot{
